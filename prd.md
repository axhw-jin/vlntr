...
- 기관은 활동 이름, 기관명, 위치(시/도 선택 + 상세 주소), 날짜, 시간, 모집 인원을 입력해 기회를 등록한다.
...
- 화면
  ...
  - `nav.js` — 모든 화면의 상단 내비게이션. 로고(홈) · 목록 · 지도 · 등록 · 마이페이지 ·
    테마 버튼 · 로그인 상태를 한 벌로 그린다. 지금 보는 화면은 색으로 표시한다.
    화면 제목은 링크가 아니고, 홈으로 가는 길은 왼쪽 로고 하나뿐이다.
    로그인 상태는 각 화면이 `setAuth()`로 알려준다 (supabase를 직접 부르지 않는다)
  - `datepicker.js` — 날짜 선택기. 목록 필터와 등록 폼이 함께 쓴다
  - `regions.js` — 시/도 목록과 위치 문자열 해석. 등록 폼의 시/도 선택과
    목록의 지역 필터가 같은 표를 봐야 해서 한 곳에 뒀다
